...
- Primary product surfaces: Stem Studio 剪辑台式三栏工作区（中央内嵌分离工作台）、项目库与结果中心、设置与许可。
- Evidence reviewed: `src/index.html`, `src/styles.css`, `src/renderer.js`, `src/main.js`, `src/lib.js`, `assets/screenshot.png`, `README.md`；用户希望从单次分离工具扩展为有持续工作流的本地创作应用。
...
## Product goals
- Goals: 让首次用户在一个屏幕内完成“选文件 → 采用推荐设置 → 开始分离”；让专业用户能将素材、处理记录、工作台结果和交付物持续保存在本地项目中。
- Non-goals: 不将桌面工具伪装成复杂的 DAW，不增加账户、云端或社交功能，不在本阶段引入新的音频模型。
- Success signals: 首屏只有一个明显的下一步；默认配置可被理解；复杂选项不干扰首次操作；用户能在下次打开时找到项目并回到任何可用结果。

## Personas and jobs
- Primary personas: 零基础的音频处理用户，以及需要批量分离/导出格式控制的创作者。
- User jobs: 从本地音频中分离声部；查看进度与结果；在工作台试听并导出混音；按项目组织批处理与交付成品。
- Key contexts of use: macOS/Windows 桌面窗口、深色环境、文件拖入与键盘操作。

## Information architecture
- Primary navigation: 左侧项目库与媒体库负责项目、文件和历史；中央画布负责项目概览、进度与结果；右侧检查器负责分离参数和开始操作。
- Core routes/screens: 首次准备向导、剪辑台式媒体工作区、项目库与项目结果、设置；分离结果工作台不是独立页面，而是中央画布的常驻内容。
- Content hierarchy: 左侧先选择项目与管理文件；右侧设置参数；中央始终呈现项目概览、空轨道、内嵌处理进度或真实结果；主操作固定在检查器可见底栏。

## Design principles
- 空间表达任务: 用固定三栏将“素材、画布、检查器”分开，让用户一眼理解操作对象和控制位置。
- 工作台即画布: 中央不使用欢迎页或二级页面；首次打开便展示待命轨道，分离完成后在同一位置加载真实可编辑音轨。
- 项目是容器: 一次处理自动归入本地项目；项目保存素材引用、分离设置、状态、可用结果与混音导出记录，但不复制原始媒体或音轨，避免占用额外空间。
- 结果可回到: 左侧项目库必须让用户一眼区分处理中、已完成和需处理项目；项目中的每一项结果可直接回到中央工作台或在访达显示。
- 一个主操作: 每个状态只突出最合适的下一步；开始分离固定在检查器可见底栏、无文件时禁用并说明原因，其他操作降级为次级按钮或折叠内容。
- 进度属于作品: 处理中的文件名、进度和预估时间在中央工作台内呈现，不与待命/结果状态分离。
- 首次准备可理解: 缺少引擎或模型时在一个可跳过的向导内解释清楚；全部就绪时收敛为简短成功态。下载必须由用户主动触发，视频与混音组件明确为按需可选。
...
- New/changed components: 项目库、项目摘要、项目结果行、媒体库、中央预览画布、检查器、固定操作区、分离结果工作台；进度阶段说明、失败恢复入口与工作台内的媒体组件安装入口。
...
- Project empty: 没有项目时解释项目会自动在首次分离时创建；项目没有结果时显示当前处理状态或下一步。
...
- Success: 单文件分离成功后自动在中央画布加载结果工作台；批量任务沿用完成 chip、打开文件夹和进入工作台入口；混音导出成功明确显示成品路径并记录到当前项目。
